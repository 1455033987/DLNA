...
public final class DLNAManager {
    ...
    static String tryTransformLocalMediaAddressToLocalHttpServerAddress(@NonNull Context context,
        ...
        try {
            newSourceUrl = getLocalHttpServerAddress(context) + URLEncoder.encode(sourceUrl.replace(Environment.getExternalStorageDirectory().getAbsolutePath(), ""),"UTF-8").replace("%2F","/");
